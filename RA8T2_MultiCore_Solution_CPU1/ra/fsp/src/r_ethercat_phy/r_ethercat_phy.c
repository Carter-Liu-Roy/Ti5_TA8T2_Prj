...
static bool ethercat_phy_targets_is_support_link_partner_ability(ethercat_phy_instance_ctrl_t * p_instance_ctrl,
                                                                 uint32_t                       line_speed_duplex);
static void ether_phy_write_io(uint32_t phy, uint32_t reg_addr, uint32_t  value);
static uint32_t ether_phy_read_io(uint32_t phy, uint32_t reg_addr);
static uint32_t ether_phy_read_esc(uint32_t phy, uint32_t reg_add);
static void ether_phy_write_esc(uint32_t phy, uint32_t reg_addr, uint32_t data);
static fsp_err_t ether_phy_reset(uint32_t phy);
 ...
 * Functions
 **********************************************************************************************************************/
uint32_t ether_phy_read(uint32_t phy, uint32_t reg_addr)
{
    return ether_phy_read_io(phy, reg_addr);
}

void ether_phy_write(uint32_t phy, uint32_t reg_addr, uint32_t data)
{
    ether_phy_write_io(phy, reg_addr, data);
}
/********************************************************************************************************************//**
...
fsp_err_t R_ETHERCAT_PHY_Read (ether_phy_ctrl_t * const p_ctrl, uint32_t reg_addr, uint32_t * const p_data)
{
     ...
     *p_data = ether_phy_read_io(p_instance_ctrl->p_ether_phy_cfg->channel, reg_addr );
//    {
...
fsp_err_t R_ETHERCAT_PHY_Write (ether_phy_ctrl_t * const p_ctrl, uint32_t reg_addr, uint32_t data)
{
    ...
    ether_phy_write_io(p_instance_ctrl->p_ether_phy_cfg->channel, reg_addr, data );
...
///////////////////////////////////////////////////////////////////////////////////////
// PHY operation by GPIO    ////////////////////
#define PIN_CFG_OUTL                IOPORT_CFG_PORT_DIRECTION_OUTPUT| IOPORT_CFG_PORT_OUTPUT_LOW
#define PIN_CFG_OUTH                IOPORT_CFG_PORT_DIRECTION_OUTPUT| IOPORT_CFG_PORT_OUTPUT_HIGH
#define PIN_CFG_IN                  IOPORT_CFG_PORT_DIRECTION_INPUT
#define PIN_CFG_PERI                IOPORT_CFG_PERIPHERAL_PIN | IOPORT_PERIPHERAL_ESC

#define MDIO_DATA                   BSP_IO_PORT_07_PIN_09
#define MDIO_CLOCK                  BSP_IO_PORT_07_PIN_08
#define mdio_delay()                R_BSP_SoftwareDelay(1, BSP_DELAY_UNITS_MICROSECONDS)


#define gpio_set_out(pin)           R_IOPORT_PinCfg(NULL, pin, PIN_CFG_OUTL)
#define gpio_set_in(pin)            R_IOPORT_PinCfg(NULL, pin, PIN_CFG_IN)


//#define mdio_out(d)                 R_BSP_PinWrite(MDIO_DATA, d);   \
//		                            mdio_delay();                   \
//		                            R_BSP_PinWrite(MDIO_CLOCK, 1);  \
//		                            mdio_delay();                   \
//		                            R_BSP_PinWrite(MDIO_CLOCK, 0);  \
//		                            mdio_delay()
void mdio_out(uint32_t d)
{
    R_IOPORT_PinWrite(NULL, MDIO_DATA, d);
    mdio_delay();
    R_IOPORT_PinWrite(NULL, MDIO_CLOCK, 1);
    mdio_delay();
    R_IOPORT_PinWrite(NULL, MDIO_CLOCK, 0);
//    mdio_delay();
}

void mdio_in(uint32_t* val)
{
    *val = R_BSP_PinRead(MDIO_DATA);
    R_IOPORT_PinWrite(NULL, MDIO_CLOCK, 1);
    mdio_delay();
    R_IOPORT_PinWrite(NULL, MDIO_CLOCK, 0);
    mdio_delay();
...
void mdio_init()
{
    R_IOPORT_PinCfg(NULL, MDIO_CLOCK, PIN_CFG_OUTL);
    R_IOPORT_PinCfg(NULL, MDIO_DATA, PIN_CFG_OUTH);
    mdio_delay();
}
...
void mdio_end()
{
    mdio_delay();
    R_IOPORT_PinWrite(NULL, MDIO_CLOCK, 0);
    R_IOPORT_PinWrite(NULL, MDIO_DATA, 1);
    R_IOPORT_PinCfg(NULL, MDIO_CLOCK, PIN_CFG_PERI);
    R_IOPORT_PinCfg(NULL, MDIO_DATA, PIN_CFG_PERI);
}
...
static void ether_phy_write_io(uint32_t phy, uint32_t reg_addr, uint32_t  value)
{
    ...
    // ?? OP code (01 for write)
    mdio_out(0);
    mdio_out(1);

    // ?? PHY address: 5bit
    for (int i = 0; i < 5; i++) {
        mdio_out((phy << i) & 0x0010);
    }

    // ?? REG address: 5bit
    for (int i = 0; i < 5; i++) {
        mdio_out( (reg_addr << i) & 0x0010);
    }

    // ?? turnaround (TA) bit: 10
    mdio_out(1);
    mdio_out(0);

    // ?? data: 16bit
    for (int i = 0; i < 16; i++) {
        mdio_out( ((value << i) & 0x8000)== 0x8000);
    }
